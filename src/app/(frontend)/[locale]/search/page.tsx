    ...
    <div className="pt-24 pb-24">
      ...
      <div className="container mb-16">
        <div className="prose dark:prose-invert max-w-none text-center">
          <h1 className="mb-8 lg:mb-16">{dictionary.search.title}</h1>
      ...
      {posts.totalDocs > 0 ? (
        ...
        <div className="container">{dictionary.search.noResults}</div>
      )}
    </div>
  )
}
...
export async function generateMetadata({ params }: Pick<Args, 'params'>): Promise<Metadata> {
  ...
  return {
    title: `قالب وب‌سایت صنعتی ${dictionary.search.title}`,
  }
}
